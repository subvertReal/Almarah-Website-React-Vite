...
function Sherwanis(){
    ...
    useEffect(() => {
        ...
        async function getShowCaseData(){
            ...
            const response = await fetch(jsonData.apiLink+'/scan/sherwanis');
    ...
    return (<>
        ...
        <Container>
            <div className='headerWhitespaceClothingGlobal'>
                <div className='headerClothingGlobal'>Sherwanis</div>
                <Container>
                    <Row>
                    {imgArr.map((image) => (
                        ...
                        <Image onClick={() => navigate(`/product/${image.id}`)} className='showcaseoneimg'
                            src={`${jsonData.apiLink}/static/groomswear/sherwanis/${image.pic1}.webp`}
                            rounded
                        />
