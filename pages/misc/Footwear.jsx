...
function Footwear(){
    ...
    useEffect(() => {
        ...
        async function getShowCaseData(){
            ...
            const response = await fetch(jsonData.apiLink+'/scan/footwear');
    ...
    return (<>
        ...
        <Container>
            <div className='headerWhitespaceClothingGlobal'>
                <div className='headerClothingGlobal'>Footwear</div>
                <Container>
                    <Row>
                    {imgArr.map((image) => (
                        ...
                        <Image onClick={() => navigate(`/product/${image.id}`)} className='showcaseoneimg'
                            src={`${jsonData.apiLink}/static/misc/footwear/${image.pic1}.webp`}
                            rounded
                        />
